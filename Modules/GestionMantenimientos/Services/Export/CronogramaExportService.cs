...
namespace GestLog.Modules.GestionMantenimientos.Services.Export
{
    public class CronogramaExportService : ICronogramaExportService
    {
        ...
        public CronogramaExportService(IGestLogLogger logger)
        {
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        }

        public async Task ExportAsync(IEnumerable<CronogramaMantenimientoDto> cronogramas, IEnumerable<SeguimientoMantenimientoDto>? seguimientos, int anio, string realizadoPor, string outputPath, CancellationToken ct)
        {
            // Ejecutar en Task.Run para evitar bloquear el hilo llamador
            await Task.Run(() =>
            {
                ...
                try
                {
                    var cronogramasList = cronogramas?.ToList() ?? new List<CronogramaMantenimientoDto>();
                    var seguimientosList = seguimientos?.ToList() ?? new List<SeguimientoMantenimientoDto>();
                    int weeksInYearExport = System.Globalization.ISOWeek.GetWeeksInYear(anio);

                    using var workbook = new XLWorkbook();

                    // ========== HOJA 1: CRONOGRAMA ==========
                    var ws = workbook.Worksheets.Add($"Cronograma {anio}");
                    ...
                    var logoCronPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Assets", "Simics.png");
                    try
                    {
                        if (File.Exists(logoCronPath))
                        {
                    ...
                    int lastWeekCol = 4 + weeksInYearExport;
                    ...
                    titleCellCron.Style.Font.FontColor = XLColor.Black;
                    ...
                    titleRangeCron.Style.Border.RightBorder = XLBorderStyleValues.Thin;
                    ...
                    for (int rr = infoStartRow; rr <= infoEndRow; rr++) ws.Row(rr).Height = 18;

                    int legendColBox = 4;
                    int legendLabelCol = 5;                    var legendItems = new (string label, XLColor color)[]
                    {
                        ...
                        ("Correctivo", EstadoSeguimientoUtils.XLColorFromTipo(TipoMantenimiento.Correctivo))
                    };

                    for (int i = 0; i < legendItems.Length; i++)
                    {
                        int r = infoStartRow + i;
                    ...
                    for (int s = 1; s <= weeksInYearExport; s++)
                    {
                        ...
                        weekCell.Style.Fill.BackgroundColor = XLColor.FromArgb(0x118938);
                    ...
                    int rowCountCron = 0;
                    foreach (var c in cronogramasList)
                    {
                        ct.ThrowIfCancellationRequested();
                        ...
                        for (int s = 1; s <= weeksInYearExport; s++)
                        {
                            if (TryGetEstadoFor(cronogramasList, seguimientosList, c.Codigo!, s, out var estado))
                            {
                                var cell = ws.Cell(currentRowCron, 5 + s);
                                if (estado.Seguimiento != null && estado.Seguimiento.TipoMtno == TipoMantenimiento.Correctivo)
                                {
                                    cell.Value = "C";
                                    cell.Style.Font.Bold = true;
                                    cell.Style.Font.FontColor = XLColor.White;
                                    cell.Style.Fill.BackgroundColor = XLColor.FromHtml("#7E57C2");
                    ...
                    if (cronogramasList.Count > 0)
                    {
                        int headerRowCron = currentRowCron - cronogramasList.Count - 1;
                    ...
                    double colAWidth = Math.Max(ws.Column(1).Width, 18);
                    double colBWidth = Math.Max(ws.Column(2).Width, 16);
                    double colCWidth = Math.Max(ws.Column(3).Width, 14);
                    double colDWidth = Math.Max(ws.Column(4).Width, 14);
                    double colEWidth = Math.Max(ws.Column(5).Width, 14);

                    ws.Column(1).Width = colAWidth + 2;
                    ...
                    ws.Range(currentRowCron, 1, currentRowCron, 5 + weeksInYearExport).Merge();
                    ...
                    foreach (var seg in seguimientosList.OrderBy(s => s.Semana).ThenBy(s => s.Codigo))
                    {
                        ...
                        descCell.Style.Alignment.WrapText = true;                        wsSeguimientos.Cell(currentRowSeg, 6).Value = seg.Responsable;

                        var estadoCell = wsSeguimientos.Cell(currentRowSeg, 7);
                        estadoCell.Value = EstadoToTexto(seg.Estado);
                        // Si es Correctivo, mostrar color morado; si no, mostrar color del estado
                    ...
                    _logger.LogInformation("[CronogramaExportService] Export completado: {Path}", outputPath);
                }
        ...
        // Helpers
        private bool TryGetEstadoFor(List<CronogramaMantenimientoDto> cronogramas, List<SeguimientoMantenimientoDto> seguimientos, string codigo, int semana, out MantenimientoSemanaEstadoDto estado)
        {
            estado = new MantenimientoSemanaEstadoDto();

            var seg = seguimientos.FirstOrDefault(s => s.Codigo == codigo && s.Semana == semana);
            if (seg != null)
            {
                estado.Seguimiento = seg;
                estado.Estado = seg.Estado;
                estado.CodigoEquipo = seg.Codigo ?? string.Empty;
                return true;
            }
